  ...
  return (
    <div>
      ...
      <Suspense fallback={<ListBodySkeleton rows={6} columns={5} />}>
        <ProductListData dict={t} />
      </Suspense>
    </div>
  );
}

async function ProductListData({ dict: t }: { dict: Dictionary }) {
          ...
          <TableBody>
            {rows.map((p) => (
              <TableRow key={p.id}>
                ...
                <TableCell className="text-end">{formatCurrency(p.unit_price, currency)}</TableCell>
          ...
          <div key={p.id} className="rounded-xl border bg-card p-4">
            ...
            <div className="mt-3 flex items-center justify-between text-sm">
              <div className="flex items-center gap-3">
                <span className="font-semibold">{formatCurrency(p.unit_price, currency)}</span>
